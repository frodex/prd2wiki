...
| 3 | `one-line-menu-ticket-tags-02a/css.txt` (v8+) if present, else `01a/…/v6.css` | **Style / layout** (design drop). When `02a/css.txt` is used, it is self-contained for status pills, report stripes, and outline tag colors; `apply_twowiki_skin.py` only **appends** the **sortable markdown thead** block from the end of `01a/…/v6.css` until that block is folded into `02a`. |
