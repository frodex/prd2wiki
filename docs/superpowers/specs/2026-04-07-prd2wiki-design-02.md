...
**Status:** Draft
**Version:** 0.2
**Preceding:** 0.1 (2026-04-07-prd2wiki-design-01.md)
**Changes from 0.1:** Expanded acronyms on first use. Elevated "Work Surface for Planning & Research" from Novel Contributions footnote to primary feature (Section 2). Reframed Purpose to emphasize dual-input persistent context. Added sidecar API requirement for agent-attached applications.
...
Both reference types are visible on every page:

```
Hard References (provenance):
  ├── SRC-RFC6749 (v1, sha256:abc...)      ← author cited this
  ├── PRD-028 (v3, sha256:def...)          ← author built on this
  └── research/auth-analysis (v2)           ← author used this

Soft References (discovered):
  ☑ PRD-099 "Session Management"            ← 0.89 similar
  ☐ project-b/auth-design                  ← dismissed
      Reason: "Different auth paradigm"
