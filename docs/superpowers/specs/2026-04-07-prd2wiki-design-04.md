...
**Status:** Draft
**Version:** 0.4
**Preceding:** 0.3 (2026-04-07-prd2wiki-design-03.md)
**Changes from 0.3:** Work surface now explicitly supports direct mutation of agentic plans by users with automatic stepped history and diffing (1.1). Added automatic schema enforcement for poorly input data (1.1). Cross-project mining expanded to include direct reuse discovery (2.4). Sidecar API reframed as primary pattern, not future phase (2.5). Impact assessment adds agent dispatch for remediation (6.4). Self-deprecation triggers on newer approved documents (6.5).
...
- A **work surface for planning and research** — captures the birth process of a PRD, not just the final document. Brainstorming loops, rejected approaches, reasoning, and anti-patterns are preserved as first-class records. Allows direct mutation of agentic plans by users, providing a stepped history automatically by design. Agents and users can diff prior versions to clearly see mutations and notes requiring incorporation into new versions of the artifact.
- A git-backed wiki with What You See Is What You Get (WYSIWYG) web User Interface (UI) for humans and Model Context Protocol (MCP) interface for agents, with automatic schema enforcement and correction for poorly input data
...
- Direct reuse discovery — "we already built this for Project Y" or "we solved this in our Project Q design" — not just pattern detection but finding existing solutions
...
The Core API supports sidecar applications — utilities that directly allow agents and humans to access the wiki as a user interface. The web UI and MCP sidecar are the first two examples of this pattern, not special cases.
...
- Attach directly to an agent process, giving it wiki access as its primary UI

The API must be designed to support new sidecar applications from the start. Any application that can speak REST to the Core API can become a wiki interface — agent-attached brainstorming UIs, CLI tools, IDE integrations, notification systems, or workflows not yet imagined.
...
3. Requests authorized/authoritative users or agents to address the issues. The system can re-fork work or launch remediation agents to investigate and propose fixes — not just flag, but actively seek resolution.

### 6.5 Self-Deprecation

If a page's sources are thoroughly invalidated, the system auto-sets `status: deprecated` with an explanation and required action. Self-deprecation also triggers when newer approved documents exist that supersede this page — the `superseded_by` field is set and the page's status reflects that it is no longer the current version of this knowledge.
