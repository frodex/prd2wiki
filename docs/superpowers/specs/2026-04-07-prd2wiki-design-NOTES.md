...
7. **Three submission modes** (verbatim / conform / integrate) with mandatory diff preview
   ...
   SO I WOULD LIKE THIS TO BE THE "WORK-SURFACE" FOR PLANNING AND RESEARCH, VALUABLE INFORMATION IS WASTED IN THE AGENTIC PLANNING AND RE-WORK PROCESS (IE BRAINSTORMING LOOPS) THAT REVEILS INTENT, RESONING AND ANTI-PATTERNS REJECTED ALONG THE PROCESS. USING THE WIKI AS AN INTERACTIVE TOOL WHEN IN SUCH (BRAINSTORMING SKILL LOOPS) PROVIDES BOTH A RECORD OF THE BIRTH PROCESS OF THE PRD BUT ALSO A EASY AND ACCESSABLE WEB UI APPROACH WITH SCOPED INPUTS FOR BOTH HUMAN AND AGENTIC INTERACTION THAT EXPANDS BEYOND THE CHAT INTERFACE.
