...
- A git-backed wiki with What You See Is What You Get (WYSIWYG) web User interface(UI) for humans and MCP interface for agents
...
**Core API** — Representational State Transfer - REST API (standard library `net/http`). Serves both the web UI and sidecars. CRUD (Create, Read, Update, Delete) for pages, search, provenance queries, branch operations, challenge management.
...
7. **Three submission modes** (verbatim / conform / integrate) with mandatory diff preview
   ...
   SO I WOULD LIKE THIS TO BE THE "WORK-SURFACE" FOR PLANNING AND RESEARCH, VALUABLE INFORMATION IS WASTED IN THE AGENTIC PLANNING AND RE-WORK PROCESS (I.E. BRAINSTORMING LOOPS) THAT REVEALS INTENT, REASONING AND ANTI-PATTERNS REJECTED ALONG THE PROCESS. USING THE WIKI AS AN INTERACTIVE TOOL WHEN IN SUCH (BRAINSTORMING SKILL LOOPS) PROVIDES BOTH A RECORD OF THE BIRTH PROCESS OF THE PRD BUT ALSO AN EASY AND ACCESSIBLE WEB UI APPROACH WITH SCOPED INPUTS FOR BOTH HUMAN AND AGENTIC INTERACTION THAT EXPANDS BEYOND THE CHAT INTERFACE.
